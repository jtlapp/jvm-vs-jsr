...
func (rdb *ResultsDB) SaveResults(
	...
	remaining *util.RemainingResources,
) error {
	...
	const query = `
		INSERT INTO results (
			...
			"resultStatusCodes",
			"resultErrorMessages",
			"remainingPortsActivePercent",
			"remainingPortsWaitingPercent",
	...
	_, err = pool.Exec(context.Background(), query,
		...
		float64(remaining.EstablishedPortsPercent), // $29 - remainingPortsActivePercent
		float64(remaining.TimeWaitPortsPercent),    // $30 - remainingPortsWaitingPercent
		float64(remaining.FDsInUsePercent),         // $31 - remainingFDsInUsePercent
	)
